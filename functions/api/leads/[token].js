...
export async function onRequestGet(context) {
  ...
  let rows;
  try { rows = await sb.select('velocity_leads', `token=eq.${token}&select=id,token,status,full_data,submitted_at,first_submitted_at,is_locked,quote_amount,is_paid,upgrade_permission,due_date,client_name,client_email,created_at,admin_comment,site_link,domain_choice,domain_name,email_verified,scope_sent_at,scope_accepted,scope_accepted_at,scope_text,msa_accepted_at,msa_accepted_ip,stripe_payment_intent_id,kickoff_date,delivery_target_date,company_stage,competitor_benchmarks,copy_readiness,visual_typography_scale,visual_layout_density,visual_chromatographic`); } catch (_) { return errRes('Service unavailable', 503); }
